...
var errorHintMap = {
    ...
    "-41505": "Network is full. Please try again later.",
    "-41506": "Invalid authorization information.",
    "-41507": "Your session timed out. Please authenticate again.",
    "-41508": "Local User data limit exceeded.",
    "-41512": "Local User account expired.",
    "-41513": "Local User account is disabled.",
    "-41514": "MAC address is incorrect.",
    "-41515": "Local User Quota exceeded.",
    "-41516": "Network is full. Please try again later.",
    "-41517": "Incorrect password.",
    "-41518": "This network does not exist.",
    "-41519": "Invalid code.",
    "-41520": "The code has expired.",
    "-41521": "Network is full. Please try again later.",
    "-41522": "Failed to validate the code.",
    "-41523": "Failed to send verification code. Check your number.",
    "-41524": "Authentication failed: Username not found.",
    "-41525": "Authentication failed: Wrong password.",
    "-41526": "Authentication failed: Invalid client.",
    "-41527": "Authentication failed: Invalid local user.",
    "-41528": "Failed to decrypt data.",
    "-41529": "Incorrect username or password.",
    "-41530": "Connection timeout. Please try again.",
    "-41531": "Your code has reached its Wi-Fi data limit.",
    "-41532": "Your account has reached its Wi-Fi data limit.",
    "-41533": "Form authentication request is invalid.",
    "-43408": "Internal error: Invalid configuration.",
    "-43409": "Internal error: Invalid credentials.",
    "-41538": "Voucher is not effective."
};

// --- Custom Onetap Features --- //
function showNotification(message, type) {
    if(!message || message === "ok") return;
    
    var ribbon = document.getElementById("notification-ribbon");
    var ribbonMessage = document.getElementById("ribbon-message");
    ...
    if(type === 'success') {
        ...
        ribbon.classList.add("ribbon-error");
    }
    
    ribbonMessage.innerText = message;
    ribbon.classList.add("ribbon-show");
    
    // Auto hide after 4 seconds
    setTimeout(function() {
        ribbon.classList.remove("ribbon-show");
    }, 4000);
}
    ...
    for(var i=0; i<toggles.length; i++) {
        toggles[i].addEventListener("click", function() {
            ...
            if(input.getAttribute('type') === 'password') {
                ...
                // Change eye to crossed eye (optional, but requested feedback)
...
// Function to safely strip all white spaces string from inputs
...
Ajax.post(
    ...
    function (res) {
        ...
        function pageConfigParse(){
            if (res.errorCode !== 0){
                showNotification(errorHintMap[res.errorCode], 'error');
            }
            // Bind custom UI functionality
            initPasswordToggles();
        ...
        function handleSubmit(){
            ...
            switch (window.authType){
                case 3:
                    submitData['voucherCode'] = sanitizeInputStr(document.getElementById("voucherCode").value);
                    break;
                case 5:
                    submitData['localuser'] = sanitizeInputStr(document.getElementById("username").value);
                    submitData['localuserPsw'] = sanitizeInputStr(document.getElementById("password").value);
                    break;
                case 1:
                    submitData['simplePassword'] = sanitizeInputStr(document.getElementById("simplePassword").value);
                    break;
                case 0:
                    break;
                case 6:
                    submitData['phone'] = "+"+sanitizeInputStr(document.getElementById("country-code").value) + sanitizeInputStr(document.getElementById("phone-number").value);
                    submitData['code'] = sanitizeInputStr(document.getElementById("verify-code").value);
                    break;
                case 2:
                case 8:
                    submitData['username'] = sanitizeInputStr(document.getElementById("username").value);
                    submitData['password'] = sanitizeInputStr(document.getElementById("password").value);
                    break;
                case 15:
                  submitData['ldapUsername'] = sanitizeInputStr(document.getElementById("username").value);
                  submitData['ldapPassword'] = sanitizeInputStr(document.getElementById("password").value);
                  break;
                ...
                function doAuth () {
                    Ajax.post(submitUrl, JSON.stringify(submitData).toString(), function(data){
                        ...
                        if(!!data && data.errorCode === 0) {
                            ...
                            showNotification("Logging in successfully...", 'success');
                            window.location.href = landingUrl;
                            
                        } else{
                            showNotification(errorHintMap[data.errorCode] || "Login failed.", 'error');
                        }
                    });
                }
                doAuth();
            ...
            function sendSmsAuthCode () {
                Ajax.post("/portal/sendSmsAuthCode",
                    JSON.stringify({
                        ...
                        radioId: radioId,
                        vid: vid,
                    phone: "+" + sanitizeInputStr(document.getElementById("country-code").value) + sanitizeInputStr(phoneNum)
                    }),function(data){
                        data = JSON.parse(data);
                        if(data.errorCode !== 0){
                            showNotification(errorHintMap[data.errorCode] || "Failed sending SMS code.", 'error');
                        } else {
                            showNotification("SMS has been sent successfully.", 'success');
                        }
                    }
                );
            }
            sendSmsAuthCode();
            showNotification("Sending Authorization Code...", 'success');
        });
        pageConfigParse();
    }
);
